...
ZFSでのRAIDZでは、データの管理単位を一定の固定サイズではなく可変サイズとして、元々コピー・オン・ライトで動作するため書き込みの際に既存データの読み出しの必要が無い。このため、RAIDZでは、データの書き込みに大きなペナルティは発生せず、特別のバッファメモリ等を利用しなくても実用上十分な書き込み速度が得られる。
